...
def main():
    ...
    if share_mode:
        print("🌐 Запуск веб-интерфейса с публичным доступом...")
        print("📡 Создается публичная ссылка для доступа из интернета")
    else:
        print("🏠 Запуск веб-интерфейса в локальном режиме...")
        print("🔒 Доступ только с этого компьютера")
    
    interface = create_interface()
    
    # Запуск веб-сервера
    interface.launch(
        share=share_mode,  # Публичный доступ если указан --share
